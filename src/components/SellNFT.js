    ...
    const findButton = () => {
        ...
        if (button && !buttonFound) {
            ...
            button.addEventListener('click', () => {
                eventSubject.emit('buttonClick', { message: 'List NFT button clicked!' });
            });
        } else {
        ...
        try {
            ...
            await transaction.wait().then(async (receipt) => {
                if (receipt && receipt.status == 1) {
                   alert("NFT succesfully uploaded on the marketplace using " + estimated_gas + " gas! Waiting for a buyer...")
                }else{
                    alert("Listing the NFT in the marketplace has failed!")
                }
            });
